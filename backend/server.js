...
const inferredFrontendRoutePath = inferMiniAppPath(
  process.env.WEBAPP_ROUTE_PATH || process.env.MINI_APP_ROUTE_PATH,
  configuredWebAppUrl
);
const configuredTelegramAppShortName = trimSlashes(
  process.env.TELEGRAM_APP_SHORT_NAME || process.env.TELEGRAM_APP_PATH || ""
);
const configuredPublishChannels = parseList(
  process.env.TELEGRAM_PUBLISH_CHANNELS || process.env.PUBLISH_CHANNELS || ""
);
const CONFIG = {
  ...
  frontendRoutePath: inferredFrontendRoutePath,
  telegramAppShortName: configuredTelegramAppShortName || inferredFrontendRoutePath,
  ...
  publishChannels: configuredPublishChannels,
};
...
async function handleApi(req, res, url, body) {
  ...
  if (method === "GET" && pathname === "/api/health") {
    sendJson(res, 200, {
      ...
      config: {
        ...
        frontendRoutePath: CONFIG.frontendRoutePath,
        telegramAppShortName: CONFIG.telegramAppShortName,
        ...
        publishChannels: CONFIG.publishChannels,
      },
    });
    return;
  }
  ...
  if (method === "POST" && pathname === "/api/raffles/clear-active") {
    assertAdmin(viewer);
    const activeRaffleIds = new Set(
      state.raffles
        .filter((raffle) => raffle.status !== "completed")
        .map((raffle) => raffle.id)
    );

    const removedCount = activeRaffleIds.size;
    state.raffles = state.raffles.filter((raffle) => raffle.status === "completed");
    state.payments = state.payments.filter((payment) => !activeRaffleIds.has(payment.raffleId));
    persistState();
  ...
  if (method === "POST" && action === "test-fund") {
    assertAdmin(viewer);
    const payment = createTestPaymentIntent(raffle);
    state.payments.push(payment);
    confirmPayment(payment, raffle, { source: "test_mock" });
    persistState();
    sendJson(res, 201, { payment, raffle: withComputedFields(raffle) });
    return;
  }
  ...
  if (method === "POST" && action === "claim-prize") {
    const result = await claimWinnerPrize(raffle, body.json || {});
    persistState();
    sendJson(res, 200, {
      ok: true,
  ...
  if (method === "POST" && action === "finalize") {
    ...
    await updateRaffleChannelPost(raffle);
    persistState();
    sendJson(res, 200, { raffle: withComputedFields(raffle) });
    return;
  }
...
function getViewerFromRequest(req) {
  ...
  const fallbackUserId = String(req.headers["x-telegram-user-id"] || "").trim();
  const verifiedUserId = verifyTelegramInitData(initData);
  const telegramUserId = verifiedUserId || fallbackUserId;

  return {
    telegramUserId,
    isAdmin: Boolean(telegramUserId) && CONFIG.adminIds.includes(String(telegramUserId)),
  };
}
...
function createRaffle(body) {
  ...
  const drawAt = body.drawAt || new Date(Date.now() + 60 * 60 * 1000).toISOString();
  const postText = String(body.postText || body.description || "").trim();
  const title = String(body.title || deriveTitleFromPost(postText) || "Розыгрыш").trim();

  if (!title || !Number.isFinite(prizePool) || prizePool < 0.1) {
    throw new Error("title and valid prizePool of at least 0.1 are required");
  }
  ...
  return {
    ...
    captcha,
    channelPost: {
      chatId: String(body.channel || "").trim() || null,
      messageId: null,
      url: null,
      publishedAt: null,
      lastUpdatedAt: null,
...
function createTestPaymentIntent(raffle) {
  return {
    id: `testpay_${crypto.randomUUID().slice(0, 8)}`,
    raffleId: raffle.id,
    amount: raffle.prizePool,
    currency: raffle.currency,
    status: "pending",
...
function joinRaffle(raffle, body) {
  ...
  if (
    raffle.captcha &&
    String(raffle.captcha.answer || "").trim() &&
    captchaAnswer !== String(raffle.captcha.answer || "").trim()
  ) {
...
async function claimWinnerPrize(raffle, body) {
  if (raffle.status !== "completed") {
    throw new Error("raffle_not_completed");
  }
  ...
  const winner = raffle.winners.find((item) => String(item.telegramId || "") === telegramId);
  if (!winner) {
  ...
  if (
    raffle.crypto?.mode === "test_mock" ||
    winner.payout.status === "pending_configuration" ||
    winner.payout.status === "skipped"
  ) {
    winner.payout.status = "completed";
    winner.payout.transferId = winner.payout.transferId || `claim_${crypto.randomUUID().slice(0, 8)}`;
...
async function publishRafflePost(raffle) {
  const channel = String(raffle.channel || "").trim();
  if (!channel || !CONFIG.telegramBotToken) {
    return;
  }
  ...
  try {
    const message = await telegramRequest("sendMessage", {
      chat_id: channel,
      text: renderChannelPostText(raffle),
      reply_markup: {
        inline_keyboard: [
          [
            {
              text: "Участвовать",
              url: buildMiniAppLink(`raffle_${raffle.slug}`),
            },
          ],
        ],
      },
    ...
    raffle.channelPost = raffle.channelPost || {};
    raffle.channelPost.chatId = channel;
    raffle.channelPost.messageId = message.message_id || null;
    raffle.channelPost.url = buildChannelMessageUrl(channel, message.message_id);
    raffle.channelPost.publishedAt = new Date().toISOString();
    raffle.channelPost.lastUpdatedAt = raffle.channelPost.publishedAt;
    raffle.channelPost.error = null;
  } catch (error) {
    raffle.channelPost = raffle.channelPost || {};
    raffle.channelPost.chatId = channel;
    raffle.channelPost.error = error.message;
  }
}

async function updateRaffleChannelPost(raffle) {
  const channel = String(raffle.channelPost?.chatId || raffle.channel || "").trim();
  const messageId = raffle.channelPost?.messageId;
  if (!channel || !messageId || !CONFIG.telegramBotToken) {
    return;
  }
  ...
  try {
    ...
    raffle.channelPost.lastUpdatedAt = new Date().toISOString();
    raffle.channelPost.error = null;
  } catch (error) {
    raffle.channelPost.error = error.message;
  }
...
function renderChannelPostText(raffle) {
  const lines = [
    `🎁 ${raffle.title}`,
    "",
    raffle.postText || raffle.description || "Новый конкурс уже открыт.",
    "",
    `Приз: ${raffle.prizePool} ${raffle.currency}`,
    `Победителей: ${raffle.winnersCount}`,
  ...
  if (raffle.status === "completed") {
    ...
    if (raffle.winners.length) {
      for (const winner of raffle.winners) {
        lines.push(`${winner.place}. ${winner.displayName} - ${winner.prize} ${raffle.currency}`);
      }
    } else {
      lines.push("Победителей нет");
    }
  } else {
    lines.push("");
    lines.push("Нажми кнопку ниже и пройди капчу в Mini App.");
  }

  return lines.join("\n");
}

function buildChannelMessageUrl(channel, messageId) {
  const normalized = String(channel || "").trim().replace(/^@/, "");
...
function buildRacePayload(raffle) {
  ...
  const racers = finalists.map((participant, index) => {
    ...
    const duration = isWinner
      ? 7600 + winnerIndex * 780
      : baseDuration + index * 240 + Math.round(Math.random() * 1400);
  ...
  return {
    ...
    world: {
      width: 1080,
      height: 2380 + finalists.length * 36,
      finishY: 2220 + finalists.length * 28,
    },
...
function buildObstacles(racersCount) {
  const obstacles = [];
  const rows = Math.max(12, racersCount + 6);

  for (let row = 0; row < rows; row += 1) {
    const count = row % 3 === 0 ? 5 : row % 2 === 0 ? 6 : 7;
    for (let col = 0; col < count; col += 1) {
      const kind = ["peg", "bumper", "spinner"][(row + col) % 3];
      obstacles.push({
        x: 96 + col * 136 + (row % 2 ? 54 : 0) + ((row * 19 + col * 11) % 22),
        y: 180 + row * 128,
        r: kind === "spinner" ? 18 : kind === "bumper" ? 24 : 14 + ((row + col) % 3),
        kind,
        drift: ((row * 13 + col * 17) % 28) - 14,
      });
    }
  }
...
function withComputedFields(raffle) {
  return {
    ...
    captcha: raffle.captcha
      ? {
          prompt: raffle.captcha.prompt,
          options: raffle.captcha.options,
        }
      : null,
    ...
    paymentLink: raffle.crypto?.invoiceUrl || null,
    isShareReady: raffle.status === "funded" || raffle.status === "completed",
...
function pickColor(index) {
  const palette = ["#3df28d", "#94ff54", "#19d46b", "#c7ff5f", "#68ffb1", "#14b85a"];
...
function serveStatic(pathname, res) {
  const miniAppPath = CONFIG.frontendRoutePath ? `/${CONFIG.frontendRoutePath}` : "";
  const requested =
...
function buildMiniAppLink(startapp) {
  ...
  const appPath = CONFIG.telegramAppShortName ? `/${CONFIG.telegramAppShortName}` : "";
  return `https://t.me/${botUsername}${appPath}?startapp=${encodeURIComponent(startapp)}`;
}

function deriveTitleFromPost(postText) {
...
function generateCaptchaChallenge() {
  const sets = [
    {
      prompt: "Выбери апельсин",
      answer: "orange",
      options: shuffleArray([
        { id: "orange", label: "Апельсин", emoji: "🍊" },
        { id: "cherry", label: "Вишня", emoji: "🍒" },
    ...
    },
    {
      ...
      answer: "lemon",
      options: shuffleArray([
        { id: "watermelon", label: "Арбуз", emoji: "🍉" },
        { id: "lemon", label: "Лимон", emoji: "🍋" },
        { id: "grapes", label: "Виноград", emoji: "🍇" },
      ]),
    },
  ];
