...
function wireAdmin() {
  ...
  updateDrawPreview();
  elements.durationMinutes.addEventListener("change", updateDrawPreview);
...
function populateDurationOptions() {
  elements.durationMinutes.innerHTML = Array.from({ length: 30 }, (_, index) => {
    const minutes = index + 1;
    const label = minutes === 1 ? "1 минута" : `${minutes} минут`;
    return `<option value="${minutes}">${label}</option>`;
  }).join("");
  elements.durationMinutes.value = "1";
}

function updateDrawPreview() {
  const durationMinutes = Number(elements.durationMinutes.value || 1);
  const finishAt = new Date(Date.now() + durationMinutes * 60 * 1000);
  elements.drawPreview.innerHTML = `
    <strong>Длительность: ${durationMinutes} мин.</strong>
    <small>Розыгрыш завершится автоматически через ${durationMinutes} мин. от текущего времени устройства.</small>
...
function renderAdminResult(raffle, payment) {
  ...
  const statusText = isFunded ? "Розыгрыш оплачен" : "Счет создан, ждем оплату";
  const helpText =
    payment.mode === "cryptopay"
      ? "Открой ссылку на инвойс и оплати счет в CryptoBot. После webhook розыгрыш перейдет в funded."
