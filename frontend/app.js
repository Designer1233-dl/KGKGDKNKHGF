  ...
  elements.joinButton.addEventListener("click", async () => {
    ...
    try {
      ...
      elements.participantResult.innerHTML = `
        ...
        <small>Твоя реферальная ссылка. Каждое приглашение дает +10 к шансу.</small>
        <code>${escapeHtml(joined.referralLink)}</code>
        ${renderLinkActions(joined.referralLink, `Залетай в розыгрыш ${joined.raffle?.title || ""}`)}
      `;

      elements.participantRefCode.value = joined.participant.refCode;
      ensureShareActions(
        elements.participantResult,
        joined.referralLink,
        `Залетай в розыгрыш ${joined.raffle?.title || ""}`
      );
...
function updateDrawPreview(selectElement, previewElement, isTest) {
  ...
  `;
  if (isFunded) {
    ensureShareActions(elements.adminResult, raffle.publicLink, `Залетай в розыгрыш ${raffle.title}`);
  }
}
...
function renderAdminResult(raffle, payment) {
  ...
  elements.adminResult.innerHTML = `
    ...
    ${shareBlock}
  `;
  ensureShareActions(elements.testAdminResult, raffle.publicLink, `Залетай в розыгрыш ${raffle.title}`);
}
...
function renderLinkActions(url, shareText) {
  ...
  return `
    <div class="link-actions">
      ...
      <button class="primary" type="button" data-share-link="${escapeHtml(url)}" data-share-text="${escapeHtml(shareText || "Открывай розыгрыш")}">Поделиться</button>
    </div>
  `;
}

function ensureShareActions(container, url, shareText) {
  if (!container || !url) {
    return;
  }

  const existing = container.querySelector(".post-render-share-actions");
  if (existing) {
    existing.remove();
  }

  const actions = document.createElement("div");
  actions.className = "link-actions post-render-share-actions";
  ...
  shareAnchor.className = "primary link-action-link";
  shareAnchor.href = buildTelegramShareUrl(url, shareText || "Открывай розыгрыш");
  shareAnchor.target = "_blank";
  shareAnchor.rel = "noopener noreferrer";
  shareAnchor.dataset.shareLink = url;
  shareAnchor.dataset.shareText = shareText || "Открывай розыгрыш";
  shareAnchor.textContent = "Поделиться";

  actions.append(copyButton, shareAnchor);
...
function findEntryRaffle() {
  ...
  return (
    ...
    state.raffles.find(
      (raffle) => raffle.publicLink && raffle.publicLink.includes(`raffle_${raffleSlug}`)
    ) ||
    null
  );
}
