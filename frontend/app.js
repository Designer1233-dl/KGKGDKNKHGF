...
function setupParticipantWelcome() {
  const participantCard = document.querySelector(".participant-entry-card");
  const participantCopy = participantCard?.querySelector(".participant-entry-copy");
  if (!participantCard || !participantCopy) {
    return;
  }

  participantCopy.innerHTML = `
    <span class="participant-kicker">Для обычных игроков</span>
    <h2>Добро пожаловать</h2>
    <p>Нажмите кнопку ниже для проверки активных розыгрышей. Если розыгрыш найден, появится пузырь с кнопкой участия.</p>
  `;

  const existingButton = document.getElementById("participant-check-button");
  const existingBubbles = document.getElementById("participant-check-bubbles");

  if (!existingButton) {
    participantCopy.insertAdjacentHTML(
      "afterend",
      `
        <button id="participant-check-button" class="secondary wide participant-check-button" type="button">Проверка активных</button>
        <div id="participant-check-bubbles" class="participant-check-bubbles hidden"></div>
      `
    );
  }

  elements.participantCheckButton = document.getElementById("participant-check-button") || existingButton;
  elements.participantCheckBubbles = document.getElementById("participant-check-bubbles") || existingBubbles;

  [
    elements.participantRaffle?.closest("label"),
    elements.participantTelegramId?.closest("label"),
    elements.participantUsername?.closest("label"),
    elements.participantDisplayName?.closest("label"),
    elements.joinButton,
  ].forEach((node) => node?.classList.add("participant-hidden-field"));
}
...
function wireParticipant() {
  ...
  elements.joinButton.addEventListener("click", handleJoin);
  elements.participantCheckButton?.addEventListener("click", () => {
    renderParticipantCheckBubbles();
  });
...
async function refreshRaffles(selectId) {
  ...
  const activeParticipantRaffle =
    (entryRaffle && entryRaffle.status === "funded" && entryRaffle) ||
    participantRaffles.find((raffle) => raffle.id === selectId) ||
    (participantRaffles.length === 1 ? participantRaffles[0] : null);
  const shouldLockParticipantFlow = !activeParticipantRaffle && !participantRaffles.length;

  if (selectId) {
    elements.participantRaffle.value = selectId;
    elements.raceRaffle.value = selectId;
  } else if (activeParticipantRaffle) {
    elements.participantRaffle.value = activeParticipantRaffle.id;
    elements.raceRaffle.value = activeParticipantRaffle.id;
  }
  ...
  if (elements.participantCheckBubbles && !participantRaffles.length) {
    elements.participantCheckBubbles.classList.add("hidden");
    elements.participantCheckBubbles.innerHTML = "";
  }
...
function renderParticipantCheckBubbles() {
  if (!elements.participantCheckBubbles) {
    return;
  }

  const activeRaffles = state.raffles.filter((raffle) => raffle.status === "funded");
  elements.participantCheckBubbles.classList.remove("hidden");

  if (!activeRaffles.length) {
    elements.participantCheckBubbles.innerHTML = `
      <div class="participant-bubble participant-bubble-empty">
        <strong>К сожалению, нету</strong>
        <small>Сейчас активных розыгрышей нет. Загляни позже.</small>
      </div>
    `;
    return;
  }

  const raffle = findEntryRaffle() || activeRaffles[0];
  elements.participantRaffle.value = raffle.id;
  elements.participantCheckBubbles.innerHTML = `
    <div class="participant-bubble participant-bubble-active">
      <span class="participant-bubble-dot"></span>
      <strong>Вот есть активный розыгрыш</strong>
      <small>${escapeHtml(raffle.title)} | Победителей: ${raffle.winnersCount}</small>
      <button id="participant-bubble-join" class="primary wide" type="button">Участвовать</button>
    </div>
  `;

  document.getElementById("participant-bubble-join")?.addEventListener("click", () => {
