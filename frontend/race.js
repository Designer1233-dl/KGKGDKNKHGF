...
export class NeonRaceRenderer {
  ...
  resizeCanvas() {
    ...
    const width = Math.max(980, Math.round(parent?.clientWidth || window.innerWidth || 1080));
    const height = Math.max(720, Math.round(parent?.clientHeight || window.innerHeight || 720));
  ...
  drawCourse(cameraY) {
    ...
    ctx.lineWidth = 1;
    for (let x = 96; x < canvas.width - 70; x += 118) {
      ctx.beginPath();
      ctx.moveTo(x, 24);
  ...
  drawRacer(racer, cameraY) {
    ...
    ctx.fillText(cropName(racer.usernameLabel || racer.displayName, 16), racer.x, screenY - 34);
    ctx.font = "600 11px Manrope, sans-serif";
    ctx.fillStyle = "rgba(255,255,255,0.78)";
    ctx.fillText(`+${racer.referralBonus || 0} chance`, racer.x, screenY - 16);
    ctx.restore();
  }
